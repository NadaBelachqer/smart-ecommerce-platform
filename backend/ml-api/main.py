...
def predict(data: ForecastRequest):
    ...
    try:
        ...
        if product_history.empty:
            print(f"⚠️ Product {data.productId} not found in history, falling back to product ID 1 context.")
            product_history = X_context[X_context["Product_ID"] == 1]
            if product_history.empty:
                raise HTTPException(status_code=404, detail="Fallback product history not found")
