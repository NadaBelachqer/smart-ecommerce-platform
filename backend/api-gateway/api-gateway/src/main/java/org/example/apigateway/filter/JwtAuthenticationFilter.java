...
public class JwtAuthenticationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, org.springframework.cloud.gateway.filter.GatewayFilterChain chain) {
        ...
        // Allow CORS preflight requests to pass without JWT check
        if (HttpMethod.OPTIONS.equals(request.getMethod())) {
            System.out.println("✅ Requête OPTIONS (preflight CORS) — passage autorisé");
        ...
        if (path.contains("/admin")) {
            ...
            if (!"ADMIN".equalsIgnoreCase(role)) {
                System.out.println("❌ Accès refusé : Rôle ADMIN requis. Rôle actuel : " + role);
                exchange.getResponse().setStatusCode(HttpStatus.FORBIDDEN);
                return exchange.getResponse().setComplete();
            }
            System.out.println("✅ Accès admin autorisé pour le rôle : " + role);
        }
